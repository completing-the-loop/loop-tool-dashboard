...
def get_prepostevent_treetable(course_id, course_weeks, treelist_json, curr_evt, course_type):
    ...
    engine = create_engine('mysql://root:password@localhost/cloop_olap?unix_socket=/Applications/MAMP/tmp/mysql/mysql.sock', echo=False)
...
def create_bins(grouped_totals_list, max, bin_allocation):
    ...
    allocations_list = []
    allocations_dict = {}
    for i in range(bin_allocation, (number_of_bins*bin_allocation)+1, bin_allocation):
